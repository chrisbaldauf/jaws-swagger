...
import { parseProject } from '../helpers/parser';
import { uploadDir } from '../helpers/aws';

var validator = new SwaggerValidator();

var debug = debugLib('JAWS-Swagger:commands:documentation');
...
export function create( projectRootPath = '.' ){
    return co(function*(){
        ...
        let json = yield parseProject( projectRootPath );

        debug( yield new Promise( (resolve, reject) => {
            validator.validate( json, (err, results) => {
                if( err ) reject( err );
                else if( results ) reject( results );
                else resolve('Project JSON created with no errors');
            });
        }).catch( (err) => {
            err.json = json;
            throw err;
    ...
    }).catch( (err) => console.log( JSON.stringify(err, null, '  ')) );
}
